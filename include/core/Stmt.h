...
public:
    ...
    return std::visit(
        [](auto &&arg) -> const Token & {
          ...
          if constexpr (std::is_same_v<T, DeclarationStmt>) {
            ...
            if (arg.expression)
              return arg.expression->get_token();
          }
          return dummy;
        },
        node);
  ...
  ExpressionStmt &get_expression_stmt() {
    return std::get<ExpressionStmt>(node);
  }
  ...
  ExpressionStmt move_expression_stmt() {
    return std::move(std::get<ExpressionStmt>(node));
  }
