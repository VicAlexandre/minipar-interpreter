...
public:
    ...
    void execute(const std::vector<std::unique_ptr<Stmt>>& statements, const std::unordered_map<std::string, FunctionStmt*>& function_table);
...
private:
    ...
    void check_numeric_operands(const Token& op, const Value& left, const Value& right);
};
