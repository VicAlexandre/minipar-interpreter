 ...
 */
using ValueType = std::variant<int ,double, std::string, bool, std::vector<double>>;
...
public:
    ...
    bool is_array() const { return std::holds_alternative<std::vector<double>>(literal); }
    ...
    bool is_valid() const {
        switch(type) {
            ...
            case TokenType::TYPE_ARRAY_NUMBER : return is_array();
    ...
    std::vector<double> get_array() const { return std::get<std::vector<double>>(literal); }
